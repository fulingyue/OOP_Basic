...
namespace _fe {
   struct Range {
      virtual ~Range() { };//range is a abstract class
   };

   template <typename T>
   struct ArrayRange : Range {//array's begin and end
      ArrayRange(const T *begin, const T *end) : iter(begin), end(end) { }
      const T *iter;
      const T *end;
   };
   //cont means the content in ()
   template <typename CType>
   struct CRange : Range {//other types' begin and end
   ...
   struct MapRange : Range {//struct a class of four elements
