...
Direction leftFrom(Direction dir) {//anti-clockwise direction 90 degrees
   return Direction((dir + 3) % 4);
}

Direction rightFrom(Direction dir) {//clockwise direction 90 degrees
   return Direction((dir + 1) % 4);
}

Direction opposite(Direction dir) {//the opposite direction
...
string directionToString(Direction dir) {//output the direction by strings rather than enum Direction
   switch (dir) {
...
std::ostream & operator<<(std::ostream & os, const Direction & dir) {
   return os << directionToString(dir);//cout direction strings
}
...
std::istream & operator>>(std::istream & is, Direction & dir) {//cin direction
